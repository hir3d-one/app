...
import { startJobSearch } from '@/actions/jobs/startSearch'; // Import the new action
import { useRouter } from 'next/navigation'; // Ensure useRouter is imported
import { toast } from 'sonner'; // For error notifications
import { Spotlight } from '@/components/ui/spotlight'; // Added for background
...
function JobsDraftBackground({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative flex flex-1 w-full items-center justify-center bg-background overflow-hidden">
      ...
      <div className="relative z-10 w-full h-full flex flex-col items-center px-4 md:p-4 overflow-y-auto">
        {children}
      </div>
    </div>
  );
...
export default function JobDraftRefinePage({ params }: { params: { id: string } }) {
  ...
  const [isActivating, setIsActivating] = useState(false); // For loading state on button
  const textareaRef = React.useRef<HTMLTextAreaElement>(null as any); // Ref for Textarea, using 'as any' to bypass strict null check for now
  ...
  const handleActivateSearch = async () => {
    ...
    try {
      // In a real app, ensure jobDraft.id is the correct one to use.
      // We use jobDraft.id (which is params.id if fetched, or initialJobDraft.id for demo)
      const result = await startJobSearch({ 
        jobDraftId: jobDraft.id, 
        jobTitle: jobDraft.title 
      });
      ...
      if (result.error) {
        ...
        router.push(`/dashboard/jobs/${jobDraft.id}?runId=${result.runId}&accessToken=${result.accessToken}`);
      } else {
    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <JobsDraftBackground>
        <div className="container mx-auto w-full max-w-screen-xl py-6 md:py-8">
          {/* Enhanced Page Header - NOW A CARD */}
          <Card className="mb-6 md:mb-8 bg-card/70 border shadow-lg rounded-xl overflow-hidden backdrop-blur-sm">
            <CardContent className="p-6">
              <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-1 text-foreground">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-2">
                ...
                <div className="flex items-center gap-2 mt-2 sm:mt-0">
                  <Button variant="outline" size="sm" className="h-8 text-foreground border-border hover:bg-muted/50">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-2/3 space-y-6">
              <Card className="bg-card border shadow-lg rounded-xl overflow-hidden">
                ...
                <CardContent className="space-y-6 p-6">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="locations" className="text-base font-medium text-foreground">Location(s)</Label>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="employmentType" className="text-base font-medium text-foreground">Employment Type</Label>
                    <Select 
                      value={jobDraft.employmentType} 
                      onValueChange={(value) => setJobDraft(prev => ({ ...prev, employmentType: value as JobDraft['employmentType'] }))}
                    >
                      <SelectTrigger id="employmentType" className="h-11 bg-background/50 border-border focus:bg-background focus:border-primary text-foreground data-[placeholder]:text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    <Label className="text-base font-medium text-foreground">Salary/Compensation Range (€)</Label>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        ...
                        <Input 
                          ...
                          className="h-11 bg-background/50 border-border focus:bg-background focus:border-primary"
                        />
                      </div>
                      <div className="space-y-1">
                ...
                <CardContent className="space-y-6 p-6">
                  <div className="space-y-4">
                    <div className="flex justify-between items-baseline">
                      <Label htmlFor="keySkills" className="text-base font-medium text-foreground">Key Skills/Technologies</Label>
                      ...
                      {jobDraft.keySkills.map((skill, index) => (
                        <div 
                          ...
                          className={cn(
                            ...
                            skill.mustHave ? "border-primary/50 bg-primary/10" : "border-border bg-background/30"
                          )}
                        >
                          <div className="flex-grow">
                            <Input 
                              ...
                              className={cn(
                                ...
                                skill.mustHave ? "font-medium text-primary" : "text-foreground"
                              )}
                          ...
                          <div className="flex items-center min-w-[120px]">
                            <div className={cn(
                              "flex h-9 items-center rounded-md border px-3 mr-2",
                              skill.mustHave ? "border-primary/40 bg-primary/5" : "border-border bg-background/50"
                            )}>
                              <Input 
                                type="number" 
                                placeholder="YoE" 
                                className="w-12 border-none bg-transparent h-auto p-0 focus-visible:ring-0 text-foreground"
                            ...
                            <div className="flex items-center space-x-2 min-w-[110px]">
                              <Checkbox 
                                ...
                                className={cn(
                                  skill.mustHave ? "border-primary data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground" : "border-border data-[state=checked]:bg-muted data-[state=checked]:text-foreground"
                                )}
                              />
                              <Label htmlFor={`mustHave-${index}`} className="text-sm cursor-pointer text-foreground">
                                Must-have
                              </Label>
                            </div>
                          </div>
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <Slider
                      ...
                      className="my-5 [&>span:first-child]:bg-primary"
                    />
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 pt-2">
                    <div className="space-y-3">
                      <Label htmlFor="educationLevel" className="text-base font-medium text-foreground">Education Level (Optional)</Label>
                      <Select
                        value={jobDraft.educationLevel}
                        onValueChange={(value) => setJobDraft(prev => ({ ...prev, educationLevel: value as JobDraft['educationLevel']}))}
                      >
                        <SelectTrigger id="educationLevel" className="h-11 bg-background/50 border-border focus:bg-background focus:border-primary text-foreground data-[placeholder]:text-muted-foreground">
                    ...
                    <div className="space-y-3">
                      <Label htmlFor="industryExperience" className="text-base font-medium text-foreground">Industry Experience (Optional)</Label>
                      <div className="flex flex-wrap gap-2 mb-3 min-h-7">
                        {jobDraft.industryExperience.map(industry => (
                          <Badge key={industry} variant="outline" className="text-sm py-1.5 pl-3 pr-2 bg-muted hover:bg-muted/80 border-border text-foreground group">
                            {industry}
                            <Button variant="ghost" size="icon" className="h-5 w-5 ml-1 rounded-full opacity-60 group-hover:opacity-100 text-muted-foreground hover:text-foreground" onClick={() => removeIndustry(industry)}>
                              <X className="h-3 w-3" />
                            </Button>
                          </Badge>
                        ))}
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button 
                          ...
                          className="gap-1 text-foreground border-border hover:bg-muted/50"
                        >
                          <Plus className="h-4 w-4" />
                          Add
                        </Button>
              ...
              <Card className="bg-card border shadow-lg rounded-xl overflow-hidden">
                ...
                <CardContent className="space-y-6 p-6">
                  <div className="space-y-3">
                    <Label htmlFor="languageRequirements" className="text-base font-medium text-foreground">Language Requirements</Label>
                    <div className="space-y-3">
                      {jobDraft.languageRequirements.map((lang, index) => (
                        <div key={index} className="flex items-center gap-3 p-3 border border-border rounded-lg bg-background/30">
                          <div className="flex-grow">
                            <Input 
                              value={lang.name}
                              onChange={(e) => {
                              ...
                              className="border-none bg-transparent px-0 h-auto text-base focus-visible:ring-0 text-foreground"
                          ...
                          >
                            <SelectTrigger className="min-w-[160px] h-9 border-border bg-background/50 focus:bg-background focus:border-primary text-foreground data-[placeholder]:text-muted-foreground">
                    ...
                    {jobDraft.languageRequirements.length === 0 && (
                      <div className="flex items-center justify-center py-6 border border-dashed border-border rounded-lg bg-muted/30">
                        <p className="text-sm text-muted-foreground">
                  ...
                  <div className="space-y-3 pt-1">
                    <Label htmlFor="keywords" className="text-base font-medium text-foreground">Keywords/Responsibilities (Advanced)</Label>
                    <Textarea 
                      ref={textareaRef} // Added ref to fix linter error
                      id="keywords" 
                      ...
                      rows={4}
                      className="resize-none bg-background/50 border-border focus:bg-background focus:border-primary text-foreground"
                    />
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div className="bg-primary/10 p-4 rounded-lg border border-primary/30 space-y-3 mt-2">
                    <div className="flex items-center">
                      <div className="h-8 w-8 rounded-full bg-primary/20 flex items-center justify-center mr-3">
                        <CheckCircle2 className="h-4 w-4 text-primary" />
                      </div>
                      <CardTitle className="text-md text-primary">Must-Have Criteria Summary</CardTitle>
                    </div>
                    <div className="pl-11">
                      <div className="space-y-2">
                        {jobDraft.keySkills.filter(s => s.mustHave).length > 0 ? (
                          <div className="space-y-2">
                            <p className="text-sm font-medium text-foreground">Skills and Experience:</p>
                            <div className="flex flex-wrap gap-2">
                              {jobDraft.keySkills.filter(s => s.mustHave).map(s => (
                                <Badge key={s.name} variant="default" className="bg-primary hover:bg-primary/90 px-3 py-1">
              ...
              <div className="sticky bottom-0 mt-8 bg-card/80 border-t border-border shadow-lg p-4 rounded-t-lg backdrop-blur-sm">
                <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
                  <Button variant="outline" className="w-full sm:w-auto order-2 sm:order-1 text-foreground border-border hover:bg-muted/50">
                  ...
                  <div className="flex gap-3 w-full sm:w-auto order-1 sm:order-2">
                    ...
                    >
                      ...
                      {!isActivating && <ChevronRight className="ml-2 h-4 w-4" />}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:w-1/3 space-y-6">
              <Card className="sticky top-6 bg-card border shadow-lg rounded-xl overflow-hidden">
                <CardHeader className="bg-muted/50 border-b px-6 py-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <CardTitle className="text-foreground">Live Candidate Preview</CardTitle>
                      <CardDescription className="text-muted-foreground">Top Matching Candidates</CardDescription>
                    </div>
                    {isPreviewRefreshing && (
                ...
                <CardContent className="p-0 max-h-[calc(100vh-180px)] overflow-y-auto"> {/* Adjusted max-h */}
                  <div className="p-2">
                    {demoCandidates.length > 0 ? (
                      <div className="divide-y divide-border">
                        {demoCandidates.map(candidate => (
                          <div key={candidate.id} className="p-4 transition-colors hover:bg-muted/50">
                            <div className="flex justify-between items-start mb-2">
                              <div>
                                <h3 className="font-medium text-foreground">{candidate.anonymizedName}</h3>
                              ...
                              <Badge 
                                ...
                                className={cn("ml-2 font-medium",
                                  candidate.matchScore >= 90 && "bg-green-500/20 text-green-300 border-green-500/50",
                                  candidate.matchScore >= 80 && candidate.matchScore < 90 && "bg-sky-500/20 text-sky-300 border-sky-500/50",
                                  candidate.matchScore < 80 && "bg-amber-500/20 text-amber-300 border-amber-500/50"
                                )}
                            ...
                            <div>
                                ...
                                {candidate.topSkills.map(skill => {
                                  ...
                                  return (
                                    <Badge 
                                      ...
                                      variant="outline" 
                                      className={cn(
                                        "text-xs py-0.5 px-1.5 border",
                                        isRequired ? "border-primary/50 bg-primary/10 text-primary" : "border-border bg-background/50 text-muted-foreground"
                                      )}
                                    >
                                      {skill}
                                      {isRequired && <span className="inline-block w-1.5 h-1.5 rounded-full bg-primary ml-1.5 align-middle"></span>}
                                    </Badge>
                                  );
                                })}
                              </div>
                              ...
                              <div className="text-xs flex items-center text-muted-foreground mt-2">
                                <span className="text-xs text-green-400 flex items-center mr-3">
                  ...
                  <div className="bg-muted/50 p-5 border-t border-border">
                    <div className="space-y-4">
                      <h3 className="font-medium text-sm text-foreground">Candidate Pool Insights</h3>
                      
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm items-center text-foreground">
                      ...
                      <div className="rounded-lg border border-border p-4 bg-background/50">
                        <h4 className="font-medium text-sm mb-2 text-foreground">Feedback & Guidance:</h4>
                        <div className="text-sm space-y-3 text-muted-foreground">
                          ...
                          {jobDraft.keySkills.filter(s => s.mustHave).length === 0 && (
                            <div className="flex items-start gap-2 pt-2 text-amber-500">
...
function getMatchScoreBadgeVariant(score: number) {
  // Adjusted to return class names for consistency with new badge styling in preview
  if (score >= 90) return "default"; // This will be styled by cn() further up
